...
function Home () {
    ...
    const [textSets, setTextSets] = useState([
        {"firstset": ['CARTER', 'SLAM DUNK', 'MCGRADY', 'PURPLE', 'DINOS', 'ROGERS CENTER']},
        {"secondset": ['ACC', 'BOSH', 'CALDERON', 'CHEVRON', 'CASEY', 'DEROZAN']},
        {"thirdset": ['LOWRY', 'UJIRI', 'SIAKAM', 'VANVLEET', 'ANUNOBY', 'IBAKA', 'BENCH MOB', '59 WINS', 'ALL NBA', '1ST SEED']},
        {"fourthset": ['SCOTIABANK', 'NURSE', 'TRADE', 'LEONARD', 'FUN GUY', 'GREEN', 'GASOL', 'DPOY', 'FOURTH']},
    ...
    return (
        <div className="homecont">
            <button className="soundbtn" type="button" onClick={HandleSound}>Sound: {muteMessage}</button>
            <div className="homeanimation">
                <div className="animationtextcont">
                    ...
                    <div className="words">
                        <Expire delay="2700"><Textra duration={100} stopDuration={240} effect="topDown" data={Object.values(textSets[0])[0]}></Textra></Expire>
                        <Expire delay="4700">{!loadingSecond && <Textra duration={75} stopDuration={190} effect="topDown" data={Object.values(textSets[1])[0]}></Textra>}</Expire>
                        <Expire delay="8100">{!loadingThird && <Textra duration={75} stopDuration={215} effect="topDown" data={Object.values(textSets[2])[0]}></Textra>}</Expire>
                        <Expire delay="11500">{!loadingFourth && <Textra duration={75} stopDuration={235} effect="topDown" data={Object.values(textSets[3])[0]}></Textra>}</Expire>
                        <Expire delay="13300">{!loadingFifth && <Textra duration={100} stopDuration={260} effect="topDown" data={Object.values(textSets[4])[0]}></Textra>}</Expire>
                        <Expire delay="15300">{!loadingSixth && <Textra duration={500} stopDuration={1300} effect="topDown" data={Object.values(textSets[5])[0]}></Textra>}</Expire>
                        <div className="north">{!loadingNorth && <div>NORTH</div>}</div>
                    </div>
                </div>
                <div className="animationmediacont">
                    <div className="animationvidcont">
                        <Expire delay="30000"><div className="player-wrapper"><ReactPlayer className="kawhivid" url={introVideo} width='90%' height='90%' playing={true} muted={isMute} /></div></Expire>
                    </div>
            ...
            <div className="homesecondpart">
                <div className="homesubinfo">
                    <div>{!loadingSubFirst && <div className="homesubfirst">The Premier Curator of Merchandise for the 2019 NBA Champions, the </div>}</div>
                    <div>{!loadingSubSec && <div className="homesubsec">Toronto Raptors</div>}</div>
                </div>
                <div className="homeshopcont">{!loadingHomeShop && <Link to="/shop"><button className="homeshop" type="button">Shop Now</button></Link>}</div>
            </div>
        </div>
    );
};
